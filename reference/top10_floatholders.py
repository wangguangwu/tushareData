...
def fetch_top10_floatholders(ts_code, start_date, end_date):
    """
    从 Tushare 获取前十大流通股东数据并返回 DataFrame
    """
...
def save_top10_floatholders_to_db(df):
    """
    将前十大流通股东数据 DataFrame 写入 MySQL 数据库，插入前检查是否已存在
    """
    # 替换无效值，以确保数据能正确插入
    ...
    try:
        with connection.cursor() as cursor:
            ...
            query_sql = f"""
                SELECT ts_code, ann_date FROM top10_floatholders
...
def fetch_and_save_top10_floatholders(ts_code, start_date, end_date):
    """
    根据 ts_code 获取并保存前十大流通股东数据
    """
    # 调用 fetch_top10_floatholders 获取前十大流通股东数据
    df = fetch_top10_floatholders(ts_code, start_date, end_date)

    # 调用 save_top10_floatholders_to_db 保存数据
    save_top10_floatholders_to_db(df)

    print(f"{ts_code} 的前十大流通股东数据已成功获取并保存")

# 使用示例
if __name__ == "__main__":
    fetch_and_save_top10_floatholders("600221.SH", "20230101", "")  # 示例调用
